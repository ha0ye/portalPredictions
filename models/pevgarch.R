...
#'
  forecast_pevgarch <- function(abundances, forecast_date, forecast_months, 
    ...
    cov_data <- dplyr::right_join(weather_data, ndvi_data, 
                                  by = "newmoonnumber")
    ...
    fcast_data <- dplyr::right_join(weather_fcast, ndvi_fcast, 
                    by = "newmoonnumber")

    covariates <- list(
                    c("maxtemp", "meantemp", "precipitation", "ndvi"),
    ...
    for (s in species){
      ...
      if (sum(species_abundance) == 0){
        ...
        for (proposed_covariates in covariates){
          ...
          prop_model <- tryCatch(
                          tscount::tsglm(species_abundance, 
                            model = model_setup, distr = "poisson",
                            xreg = predictors, link = "log"),
                          error = function(x) {NA})
          prop_model_aic <- tryCatch(AIC(prop_model), 
                              error = function(x) {Inf})
          prop_forecast <- tryCatch(
                             predict(prop_model, num_forecast_newmoons, 
                               level = CI_level, newxreg = fcast_predictors),
                             error = function(x) {NA})
  ...
  weather_fcast <- fcast_weather(moons = w_fc_nms, lag = 0, lead_time = lead)
  weather_all <- dplyr::bind_rows(weather, weather_fcast)
  weather_all_lag <- lag_data(weather_all, lag = 6, tail = TRUE)

  ndvi_fcast <- portalr::fcast_ndvi(ndvi, "newmoon", lead = lead, fc_nms)
  ndvi_all <- dplyr::bind_rows(ndvi, ndvi_fcast)
  ...
  forecasts_all <- forecast_pevgarch(abundances = all, 
                     ...
                     forecast_newmoons = forecast_newmoons, level = "All",
                     num_forecast_newmoons = num_fcast_nmoons, CI_level = 0.9,
                     weather_data = weather_fit_lag,
                     weather_fcast = weather_fcast_lag,
                     ndvi_data = ndvi_fit_lag, ndvi_fcast = ndvi_fcast_lag)
  ...
  forecasts_controls <- forecast_pevgarch(abundances = controls, 
                          ...
                          CI_level = 0.9, weather_data = weather_fit_lag,
                          weather_fcast = weather_fcast_lag,
