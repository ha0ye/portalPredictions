...
#' poisson environmental variable Generalized Auto-Regressive Conditional
...
forecast_pevgarch <- function(abundances, forecast_date, forecast_months, 
  ...
  cov_data <- dplyr::right_join(weather_data, ndvi_data, by = "newmoonnumber")
  ...
  for(s in species){
    ...
    if(sum(species_abundance) == 0){
      ...
      for(proposed_covariates in covariates){
        ...
        prop_model <- tryCatch(tscount::tsglm(species_abundance, 
                                              model = model_setup, 
                                              distr = "poisson",
                                              xreg = predictors, 
                                              link = "log"),
...
weather_fcast <- fcast_weather(moons = w_fc_nms, lag = 0, 
                               lead_time = ncfnm - 6)
weather_all <- dplyr::bind_rows(weather, weather_fcast)
weather_all_lag <- lag_data(weather_all, lag = 6, tail = TRUE)
ndvi_fcast <- portalr::fcast_ndvi(ndvi, "newmoon", lead = ncfnm - 6, fc_nms)
