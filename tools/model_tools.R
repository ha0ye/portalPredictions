...
interpolate_abundance <- function(abundance){
  ...
  non_sp_cn <- c("period", "total", "newmoonnumber", "year", "month")
  which_non_sp_cn <- which(colnames(abundance) %in% non_sp_cn)
  species <- colnames(abundance)[-which_non_sp_cn]
...
lag_data <- function(data, lag, tail = FALSE){
  ...
  if (tail == FALSE){
    ...
    data <- dplyr::right_join(data, hist_moons_table, by = nm_match) 
    data <- data[-(1:lag), ]
  }
...
#' 
#' @param climate_model Individual climate models available are CFSv2, CMC1, 
#'  CMC2, GFDL-FLOR, GFDL, NASA, NCAR and ENSMEAN (mean of all models).
#' @param lead_time the newmoons into the future to obtain forecasts. Max 7
...
get_climate_forecasts <- function(climate_model = "ENSMEAN", 
  ...
  if(!lead_time %in% 1:7){
    stop_msg <- paste0("Lead time must be an integer 1 - 7, got: ", lead_time)
    stop(stop_msg)
  }
  ...
  last_moon <- dplyr::select(last_moon, colnames(future_moons))
  
  st_date <- strftime(last_moon$newmoondate, format = "%Y-%m-%d")
  start_time <- as.character(as.Date(st_date) + 1)
  ...
  full_urls <- paste0(base_url, climate_model, "_forecast_", type_urls, 
                 ...
                 "&longitude=", lon, "&time_start=", start_time, "&time_end=",
                 end_time, "&accept=csv")
  ...
  hist_date <- paste(historic$year, historic$month, historic$day, sep = "-")
  historic$date <- as.Date(hist_date)
  ...
  if(n_avail_historic > 0){
    ...
    daily_forecasts$mintemp[in_fcast] <- historic$mintemp[in_hist]
    daily_forecasts$meantemp[in_fcast] <- historic$meantemp[in_hist]
    daily_forecasts$maxtemp[in_fcast] <- historic$maxtemp[in_hist]
    daily_forecasts$precipitation[in_fcast] <- historic$precipitation[in_hist]
  }
  ...
  for (i in 1:lead_time){
    as_dates <- seq.Date(newmoon_start[i] + 1, newmoon_end[i], 1)
    temp_dates <- as.character(as_dates)
  ...
  summarizing <- quos(mintemp = min(mintemp, na.rm = T), 
...
#' Build a weather forecast for model predictions by combining portal station 
#' data, climate forecasts, and historic means, impose specified lag
#' 
