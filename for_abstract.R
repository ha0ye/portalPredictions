...
t4 <- rep(NA, n_newmoons)
t11 <- rep(NA, n_newmoons)
t14 <- rep(NA, n_newmoons)
t17 <- rep(NA, n_newmoons)
...
for(i in 1:n_newmoons){
  ...
  ref <- which(plot_data$newmoonnumber == newmoons[i] & 
               plot_data$plot == 4)
  if(length(ref) > 0){
    t4[i] <- plot_data[ref, "total"]
  }
  ref <- which(plot_data$newmoonnumber == newmoons[i] & 
               plot_data$plot == 11)
  if(length(ref) > 0){
    t11[i] <- plot_data[ref, "total"]
  }
  ref <- which(plot_data$newmoonnumber == newmoons[i] & 
               plot_data$plot == 14)
  if(length(ref) > 0){
    t14[i] <- plot_data[ref, "total"]
  }
  ref <- which(plot_data$newmoonnumber == newmoons[i] & 
               plot_data$plot == 17)
  if(length(ref) > 0){
    t17[i] <- plot_data[ref, "total"]
  }
}
